...
from foundation_cms.legacy_apps.reports.views import (
    PageTypesReportView,
    _get_locale_choices,
)
...
class PageTypesReportViewTest(WagtailpagesTestCase):
    ...
    def test_queryset_filtering(self):
        ...
        self.assertIn(ContentType.objects.get_for_model(BlogPage), queryset)
    ...
    def test_queryset_ordering(self):
        ...
        blog_page_content_type = ContentType.objects.get_for_model(BlogPage)
        blog_page_position = queryset_list_pks.index(blog_page_content_type.pk)
        ...
        # Assert that the BlogPage comes before Homepage, since it has more entries
        self.assertTrue(blog_page_position < homepage_position)
        # Assert entry counts
        self.assertEqual(queryset.get(id=blog_page_content_type.pk).count, 3)
    ...
    def test_queryset_last_edited_page(self):
        ...
        # Create some blog pages:
        blog_page_a = BlogPageFactory(parent=self.homepage)
        blog_page_b = BlogPageFactory(parent=self.homepage)
        blog_page_c = BlogPageFactory(parent=self.homepage)
        # Edit the first blog page
        revision = blog_page_a.save_revision()
        revision.publish()
        # Edit the second blog page
        revision = blog_page_b.save_revision()
        revision.publish()
        # Edit the third blog page
        revision = blog_page_c.save_revision()
        revision.publish()
        # Re-edit the first blog page
        revision = blog_page_a.save_revision()
        revision.publish()
        # Get the queryset:
        queryset = self.view.decorate_paginated_queryset(self.view.get_queryset())
        # Assert that the first blog page is the last edited page
        blog_page_a.refresh_from_db()
        self.assertEqual(queryset[0].last_edited_page.id, blog_page_a.id)

    def test_queryset_last_edited_by(self):
        """Tests that the queryset correctly returns the last edited by user."""
        # Create some blog pages:
        blog_page_a = BlogPageFactory(parent=self.homepage)
        BlogPageFactory(parent=self.homepage)
        BlogPageFactory(parent=self.homepage)
        # Create some users:
        ...
        # Edit the first blog page with user_a
        revision = blog_page_a.save_revision(user=user_a)
        revision.publish(user=user_a)
        # Re-edit the first blog page with user_b
        revision = blog_page_a.save_revision(user=user_b)
        ...
        blog_page_a.refresh_from_db()
        self.assertEqual(queryset[0].last_edited_page.id, blog_page_a.id)
        # Assert that the first blog page was last edited by user b
...
class PageTypesReportFiltersTests(WagtailpagesTestCase):
    ...
    def test_all_locales_shown_if_no_filter(self):
        ...
        # Create a blog page in default locale
        blog_page = BlogPageFactory(parent=self.homepage)
        ...
        blog_page.copy_for_translation(self.fr_locale)

        # Edit the blog page in English to make sure that it's the latest
        blog_page.title = "Updated English title"
        revision = blog_page.save_revision()
        blog_page.publish(revision)
    ...
    def test_filter_by_locale(self):
        ...
        self.assertEqual(blogpage_row.count, 1)
        # The last edited page should be the French version (even though blog page was later edited in English)
        self.assertEqual(homepage_row.last_edited_page.locale, self.fr_locale)
        self.assertEqual(blogpage_row.last_edited_page.locale, self.fr_locale)
