...
def assign_random_categories(product):
...
def set_random_review_date(product):
    if "Percy" not in product.title:
        start_date = date(2020, 10, 1)
        end_date = date(2021, 1, 30)
        time_between_dates = end_date - start_date
        days_between_dates = time_between_dates.days
        random_number_of_days = randrange(days_between_dates)
        product.review_date = start_date + timedelta(days=random_number_of_days)
        product.save_revision()


def set_random_creepiness(product):
    product.get_or_create_votes()
    single_vote = [0, 0, 0, 0, 1]
    shuffle(single_vote)
    product.votes.set_votes(single_vote)
    product.creepiness_value = randint(0, 100)
    product.save_revision()
...
def generate(seed):
    ...
    print('Finalizing random data')
    for product_page in product_pages:
        assign_random_categories(product_page)
        set_random_review_date(product_page)
        set_random_creepiness(product_page)
