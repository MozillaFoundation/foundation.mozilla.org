# Generated by Django 3.1.11 on 2021-10-25 17:18
...
The only difference between ListBlock items and StreamBlock items are that
...
def convert_listblock_to_streamblock(apps, schema):
    ...
    for page in MozfestPrimaryPage.objects.all():
        ...
        # If this page doesn't have a Space Card List that needs updates, don't
        # save it. This will speed up how fast this task can run.
        needs_saving = False
        ...
        if page.body:
            ...
            if hasattr(page.body, 'raw_data'):
                for block in page.body.raw_data:
                    # If the block type is a Space Card List, iterate over each
                    # of the `space_cards`
                    if 'type' in block and block['type'] == 'space_card_list':
                        new_space_cards = []

                        for space_card in block['value']['space_cards']:
                            """
                            ...
                            ```
                            """
                            new_space_cards.append({
                                'type': 'space_card',
                                'value': dict(space_card),
                                'id': str(uuid.uuid4()),
                            })


                        if new_space_cards:
                            # Save StreamBlock-formatted Space Cards
                            block['value']['space_cards'] = new_space_cards
